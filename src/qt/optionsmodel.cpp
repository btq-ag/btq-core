...
/** Call node.updateRwSetting() with BTQ 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BTQ 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/btq/btq/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because btq 22.x will not try to read these.
...
//! Convert enabled/size values to btq -prune setting.
...
//! Get pruning enabled value to show in GUI from btq -prune setting.
...
//! Get pruning size value to show in GUI from btq -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBTQUnit")) {
        settings.setValue("DisplayBTQUnit", QVariant::fromValue(BTQUnit::BTQ));
    }
    QVariant unit = settings.value("DisplayBTQUnit");
    if (unit.canConvert<BTQUnit>()) {
        m_display_btq_unit = unit.value<BTQUnit>();
    } else {
        m_display_btq_unit = BTQUnit::BTQ;
        settings.setValue("DisplayBTQUnit", QVariant::fromValue(m_display_btq_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if btq.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_btq_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BTQUnit>() == m_display_btq_unit) return;
    m_display_btq_unit = new_unit.value<BTQUnit>();
    QSettings settings;
    settings.setValue("DisplayBTQUnit", QVariant::fromValue(m_display_btq_unit));
    Q_EMIT displayUnitChanged(m_display_btq_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/btq/btq/pull/8273
    ...
    // (https://github.com/btq-core/gui/issues/567).
