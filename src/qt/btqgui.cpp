...
const std::string BTQGUI::DEFAULT_UIPLATFORM =
...
BTQGUI::BTQGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BTQGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BTQGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BTQGUI::showModalOverlay);
...
void BTQGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BTQ address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and btq: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BTQGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BTQGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BTQGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BTQ addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BTQ addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed BTQ Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed BTQ Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BTQ command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BTQGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BTQGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BTQGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BTQGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BTQGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BTQGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BTQGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BTQGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &BTQGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BTQGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BTQGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BTQGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BTQGUI::showDebugWindow);
}

void BTQGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BTQGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BTQGUI::setCurrentWalletBySelectorIndex);
...
void BTQGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BTQGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BTQGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BTQGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BTQGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BTQGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void BTQGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BTQGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BTQGUI::removeWallet);
...
WalletController* BTQGUI::getWalletController()
{
    return m_wallet_controller;
}

void BTQGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BTQGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BTQGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BTQGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BTQGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BTQGUI::incomingTransaction);
    connect(this, &BTQGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BTQGUI::removeWallet(WalletModel* walletModel)
{
...
void BTQGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BTQGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BTQGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BTQGUI::setWalletActionsEnabled(bool enabled)
{
...
void BTQGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BTQGUI::toggleHidden);
...
void BTQGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BTQGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BTQGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BTQGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BTQGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BTQGUI::openClicked()
{
...
void BTQGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BTQGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BTQGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BTQGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BTQGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BTQGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BTQGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BTQGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to BTQ network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BTQGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BTQGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BTQGUI::updateHeadersSyncProgressLabel()
{
...
void BTQGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BTQGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BTQGUI::quitRequested);
...
void BTQGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BTQGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &BTQGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BTQGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BTQGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BTQGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BTQGUI::show);
                e->ignore();
            }
        }
    }
...
void BTQGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BTQGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BTQGUI::incomingTransaction(const QString& date, BTQUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BTQUnits::formatWithUnit(unit, amount, true));
...
void BTQGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BTQGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BTQGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BTQGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BTQGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BTQGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BTQGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BTQGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BTQGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BTQGUI::detectShutdown()
{
...
void BTQGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BTQGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BTQGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BTQGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BTQGUI::subscribeToCoreSignals()
{
...
void BTQGUI::unsubscribeFromCoreSignals()
{
...
bool BTQGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BTQUnit> units = BTQUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BTQUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BTQUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BTQUnit u : BTQUnits::availableUnits()) {
        menu->addAction(BTQUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BTQUnit newUnits)
{
    setText(BTQUnits::longName(newUnits));
}
