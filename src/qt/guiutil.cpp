...
#include <qt/btqaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BTQ address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BTQAddressEntryValidator(parent));
    widget->setCheckValidator(new BTQAddressCheckValidator(parent));
}
...
bool parseBTQURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no btq: URI
    if(!uri.isValid() || uri.scheme() != QString("btq"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BTQUnits::parse(BTQUnit::BTQ, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBTQURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBTQURI(uriInstance, out);
}

QString formatBTQURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("btq:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BTQUnits::format(BTQUnit::BTQ, info.amount, false, BTQUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBTQConf()
{
    ...
    /* Open btq.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::BTQMAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BTQ.lnk";
    if (chain == ChainType::BTQTEST) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BTQ (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BTQ (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::BTQMAIN)
        return GetAutostartDir() / "btq.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("btq-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a btq.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::BTQMAIN)
            optionFile << "Name=BTQ\n";
        else
            optionFile << strprintf("Name=BTQ (%s)\n", ChainTypeToString(chain));
