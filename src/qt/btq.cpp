...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BTQUnit>("BTQUnit");
#else
    qRegisterMetaType<BTQUnit>("BTQUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. btq_de.qm (shortcut "de" needs to be defined in btq.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. btq_de_DE.qm (shortcut "de_DE" needs to be defined in btq.qrc)
...
static const char* qt_argv = "btq-qt";

BTQApplication::BTQApplication()
...
void BTQApplication::setupPlatformStyle()
{
...
#ifdef ENABLE_WALLET
void BTQApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BTQApplication::createOptionsModel(bool resetSettings)
{
...
void BTQApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BTQGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BTQGUI::quitRequested, this, &BTQApplication::requestShutdown);
...
void BTQApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BTQApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BTQApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BTQApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BTQApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BTQApplication::handleRunawayException);
    connect(this, &BTQApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BTQApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BTQApplication::parameterSetup()
{
...
void BTQApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BTQApplication::requestInitialize()
{
...
void BTQApplication::requestShutdown()
{
...
void BTQApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // btq: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BTQGUI::handlePaymentRequest);
            connect(window, &BTQGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BTQApplication::handleRunawayException(const QString &message)
{
...
void BTQApplication::handleNonFatalException(const QString& message)
{
...
WId BTQApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BTQApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BTQGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse btq.conf, determine network, switch to network specific
    ...
    // btq: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
