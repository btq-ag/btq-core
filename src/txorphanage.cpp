...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    if (m_txid_to_wtxid.count(hash)) {
        LogPrint(BCLog::TXPACKAGES, "ignoring orphan with duplicate txid %s (new wtxid %s)\n", hash.ToString(), wtxid.ToString());
        return false;
    }
...
CTransactionRef TxOrphanage::GetTx(const GenTxid& gtxid) const
{
    LOCK(m_mutex);
    const uint256 wtxid = gtxid.IsWtxid() ? gtxid.GetHash() : ResolveWtxid(gtxid.GetHash());
    if (wtxid.IsNull()) return nullptr;
    auto it = m_orphans.find(wtxid);
    return it != m_orphans.end() ? it->second.tx : nullptr;
}
