...
//
// The btq-chainstate executable serves to surface the dependencies required
// by a program wishing to use BTQ Core's consensus engine as it is right
// now.
//
// DEVELOPER NOTE: Since this is a "demo-only", experimental, etc. executable,
//                 it may diverge from BTQ Core's coding style.
//
// It is part of the libbtqkernel project.
...
int main(int argc, char* argv[])
{
    ...
    for (std::string line; std::getline(std::cin, line);) {
        ...
        case BlockValidationResult::BLOCK_TIME_FUTURE:
            std::cerr << "block timestamp was > 15 minutes in the future (or our clock is bad)" << std::endl;
            break;
