...
RPCHelpMan importaddress()
{
    return RPCHelpMan{"importaddress",
            ...
            "Note: This command is only compatible with legacy wallets. Use \"importdescriptors\" for descriptor wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BTQ address (or hex-encoded script)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BTQ address or script");
        }
    }
    if (fRescan)
    {
    ...
    bool fGood = true;
    {
        ...
        while (file.good()) {
            ...
            if (line.empty())
                continue;
            if (line[0] == '#') {
                if (line.find("End of dump") != std::string::npos) saw_end_of_dump = true;
                continue;
            }
        ...
        if (!saw_end_of_dump) {
            ...
            throw JSONRPCError(RPC_WALLET_ERROR, "dump file missing End of dump footer");
        }
        ...
        for (const auto& meta : p2mr_metas) {
            ...
            if (!created) {
                throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Error importing P2MR metadata: %s", util::ErrorString(created).original));
            }
            progress++;
        }
        pwallet->chain().showProgress("", 100, false); // hide progress dialog in GUI
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The btq address for the private key"},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
     ...
     * https://github.com/btq/btq/issues/9934
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see btqd log for details) and could "
