...
const char * const BTQ_CONF_FILENAME = "btq.conf";
const char * const BTQ_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //btq-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BTQ_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BTQ
    // macOS: ~/Library/Application Support/BTQ
    // Unix-like: ~/.btq
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTQ";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BTQ";
#else
    // Unix-like
    return pathRet / ".btq";
#endif
