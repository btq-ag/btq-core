...
BOOST_AUTO_TEST_CASE(BlockPolicyEstimates)
{
    ...
    while (blocknum < kTrainBlocks) {
        ...
        if (blocknum == kEarlyCheck) {
            // After 30 blocks the medium horizon has one scale-20 bucket.
            // SUFFICIENT_FEETXS is 0.01 now, so the old 9*baseRate sandwich
            // from scale-2 / 0.1 no longer holds. Same as the later checks:
            // require a real estimate at one medium period.
            BOOST_CHECK(feeEst.estimateFee(1) == CFeeRate(0));
            BOOST_CHECK(feeEst.estimateFee(kMedScale) > CFeeRate(0));
        }
    }
