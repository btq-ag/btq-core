...
BOOST_AUTO_TEST_CASE(package_limits_are_one_default_template)
{
    BOOST_CHECK_EQUAL(DEFAULT_ANCESTOR_SIZE_LIMIT_KVB, 474U);
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    // The three parents pay 4.8, 4.9 and 5 BTQ (parent3 keeps its whole coinbase
    // less low_fee_amt), so paying out all but 0.1 BTQ leaves the package fee the
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    const CAmount coinbase_value{5 * COIN}; // BTQ: 5 BTQ per block
    ...
    // Now that the child's fees have "increased" by 1 BTQ, the cheap package should succeed.
    {
    ...
    // Parent pays 1 BTQ and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
