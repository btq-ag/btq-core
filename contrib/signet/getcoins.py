...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing btq-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='btq-cli', help='btq-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BTQ address to which the faucet should send')
...
parser.add_argument('btq_cli_args', nargs='*', help='Arguments to pass on to btq-cli (default: -signet)')

args = parser.parse_args()

if args.btq_cli_args == []:
    args.btq_cli_args = ['-signet']


def btq_cli(rpc_command_and_params):
    argv = [args.cmd] + args.btq_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = btq_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = btq_cli(['getnewaddress', 'faucet', 'bech32'])
