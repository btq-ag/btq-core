...
_btq_cli() {
    ...
    # save and use original argument to invoke btq-cli for -help, help and RPC
    # as btq-cli might not be in $PATH
    btq_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($btq_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_btq_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
