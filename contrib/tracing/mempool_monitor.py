...
""" Example logging BTQ Core mempool events using the mempool:added,
...
def main(btqd_path):
    btqd_with_usdts = USDT(path=str(btqd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    btqd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    btqd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    btqd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    btqd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[btqd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/btqd")
        exit(1)
