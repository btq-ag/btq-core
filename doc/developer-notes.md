...
- [Development guidelines](#development-guidelines)
    - [General BTQ Core](#general-btq-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BTQ Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while btqd is running with the `logging`
RPC.  For instance, launching btqd with `-debug` or `-debug=1` will turn on
...
with "play btqs" on a test network.
...
BTQ Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run btqd.

You can then use the `-debug=lock` configuration option at btqd startup or
`btq-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `btq-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_btq
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_btq --log_level=test_suite
$ valgrind -v --leak-check=full src/btqd -printtoconsole
...
# A coverage report will now be accessible at `./test_btq.coverage/index.html`,
...
To profile a running btqd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep btqd` -- sleep 60
```
...
BTQ Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`btqd`)](https://doxygen.btqcore.org/btqd_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `btqd.cpp`. Responsible for starting up and
  shutting down the application.

- [Init load (`b-initload`)](https://doxygen.btqcore.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
...
- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.btqcore.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.btqcore.org/httpserver_8cpp.html#abb9f6ea8819672bd9a62d3695070709c)
  : Libevent thread to listen for RPC and REST connections.

- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.btqcore.org/httpserver_8cpp.html#aa6a7bc27265043bc0193220c5ae3a55f)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txindex`, etc)](https://doxygen.btqcore.org/class_base_index.html#a96a7407421fbf877509248bbe64f8d87)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.btqcore.org/class_c_scheduler.html#a14d2800815da93577858ea078aed1fba)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.btqcore.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
  : Libevent thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.btqcore.org/class_c_connman.html#aacdbb7148575a31bb33bc345e2bf22a9)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.btqcore.org/class_c_connman.html#aa7c6970ed98a4a7bafbc071d24897d13)
    : Loads addresses of peers from the DNS.

  - ThreadMapPort (`b-mapport`)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.btqcore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 8333.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.btqcore.org/class_c_connman.html#a0b787caf95e52a346a2b31a580d60a62)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.btqcore.org/class_c_connman.html#a55e9feafc3bab78e5c9d408c207faa45)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.btqcore.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
...
However, in open source software such as BTQ Core, where everyone uses
...
pay attention to for reviewers of BTQ Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: BTQ Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `BTQ_FOO_BAR_H`, e.g.

```c++
#ifndef BTQ_FOO_BAR_H
#define BTQ_FOO_BAR_H
...
#endif // BTQ_FOO_BAR_H
```
...
Some of these are maintained by active developers of BTQ Core, in which case
...
a BTQ Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with BTQ's `select()` loop, because
...
compatibility between nodes. This happened in BTQ 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to BTQ's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `btq-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `btq-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BTQ Core, the wallet was always
