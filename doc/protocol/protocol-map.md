...
| Difficulty adjustment | Bitcoin retarget before activation; LWMA after the height-gated activation, with a 144-block averaging window. Activation at height 1 in the mainnet parameter set, so no mainnet block retargets the Bitcoin way; block 300,000 on the public testnet. The window history of released binaries is recorded in `doc/protocol/historical-validation.md` | `src/pow.cpp`; `src/consensus/params.h`; `src/kernel/chainparams.cpp` |
| Script element and transaction limits | 15,000-byte maximum script element, in every script version. 100,000-byte maximum script and 201-opcode limit, in legacy and witness-v0 only: a P2MR leaf is subject to neither, except that `OP_CHECKMULTISIGDILITHIUM` still charges its key count against the 201. 400,000-weight-unit standard transaction limit | `src/script/script.h`; `src/script/interpreter.cpp`; `src/policy/policy.h` |
...
| Sighash and dispatch | Explicit Dilithium opcode dispatch; pre-P2MR witness-v0 keyhash dispatch by public-key size, disabled under the P2MR-only rule; signer appends sighash byte. Historical paths are recorded in `doc/protocol/historical-validation.md` | `src/script/sign.cpp`; `src/script/interpreter.cpp` |
| Dilithium activation | Two height-activated buried deployments: one gating Dilithium verification, one confining the Dilithium opcodes to P2MR tapscript and disabling witness-v0 Dilithium routing. Both at height 1 in the mainnet parameter set. On the public testnet the verification deployment is at height 1 and the P2MR-only deployment is unscheduled, so the pre-P2MR regime still governs that chain unless `-testnetdilithiump2mrheight` schedules it. P2MR itself is enabled unconditionally, on every chain. Historical paths are recorded in `doc/protocol/historical-validation.md` | `src/consensus/params.h`; `src/validation.cpp`; `src/kernel/chainparams.cpp`; `src/chainparams.cpp` |
...
| Wallet RPC surface | `getnewdilithiumaddress`, `importdilithiumkey`, `signmessagewithdilithium`, `verifymessagewithdilithium`. Deprecated: `verifydilithiumsignature`, superseded by `verifymessagewithdilithium` over argument order, and `signtransactionwithdilithium`, which now only throws. No `dumpdilithiumkey`. P2MR flow: `getnewp2mraddress`, `sendtop2mr`, `listp2mr`, `getp2mrinfo`, `createp2mrspend`, `signp2mrtransaction`, `testp2mrtransaction` | `src/wallet/rpc/dilithium.cpp`; `src/wallet/rpc/p2mr.cpp`; registered in `src/wallet/rpc/wallet.cpp` |
