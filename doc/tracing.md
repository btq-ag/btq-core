# User-space, Statically Defined Tracing (USDT) for BTQ Core

BTQ Core includes statically defined tracepoints to allow for more
...
**Note**: BTQ Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BTQ Core
...
Multiple tools can list the available tracepoints in a `btqd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BTQ Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/btqd
stap net        outbound_message 0x0000000000107c05 /src/btqd
stap validation block_connected  0x00000000002fb10c /src/btqd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BTQ Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/btqd | grep NT_STAPSDT -A 4 -B 2
