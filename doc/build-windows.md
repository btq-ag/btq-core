...
Below are some notes on how to build BTQ Core for Windows.

The options known to work for building BTQ Core on Windows are:
...
Note that for WSL the BTQ Core source path MUST be somewhere in the default mount file system, for
example /usr/src/btq, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\btq`, for example:

    make install DESTDIR=/mnt/c/workspace/btq
