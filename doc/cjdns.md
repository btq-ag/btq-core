# CJDNS support in BTQ Core

It is possible to run BTQ Core over CJDNS, an encrypted IPv6 network that
...
done outside of BTQ Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BTQ network and individual nodes.
...
before it will work with your BTQ Core node. This is described in steps
...
## Run BTQ Core with CJDNS

Once you are connected to the CJDNS network, the following BTQ Core
...
When enabled, this option tells BTQ Core that it is running in an
...
IPv6 local network. This helps BTQ Core perform better address management:
...
CJDNS support was added to BTQ Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `btq-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/btq/btq/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BTQ Core

There are several ways to see your CJDNS address in BTQ Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `btq-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `btq-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `btq-cli help getnodeaddresses` for details.
