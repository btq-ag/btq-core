...
BTQ Core version 0.20.0 is now available from:

  <https://btqcore.org/bin/btq-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from BTQ Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BTQ Core version 0.18.0.
...
- Testing or debugging of implementations of the BTQ P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BTQ Core. BTQ Core logs debug messages
...
- OpenSSL is no longer used by BTQ Core.  (#17265)

- BIP70 support has been fully removed from BTQ Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/btq/btq/issues/16599),
  [PR #16702](https://github.com/btq/btq/pull/16702), and the
  `btqd help` for more information.  This option is experimental and
...
- All custom settings configured when BTQ Core starts are now
...
- The "Start BTQ Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://btqcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple BTQ Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BTQ Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/btq-core/HWI) for
  signing.  Future versions of BTQ Core should support a GUI option
...
- BTQ Core's automatically-generated source code documentation is
  now available at https://doxygen.btqcore.org.  (#17596)
...
- The `btq-cli` utility used with the `-getinfo` parameter now
...
- The `btq-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `btqstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_btq-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBTQ_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_btq` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBTQ_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for btq-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate btq schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest btq.conf (sdaftuar)
...
- #18022 Fix appveyor `test_btq` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (btq-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone btq-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BTQ Core files (hebasto)
...
- #17119 Fix broken btq-cli examples (andrewtoth)
...
- #17503 Remove bitness from btq-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.btqcore.org (laanwj)
...
- #17648 Rename wallet-tool references to btq-wallet (hel-o)
...
- #18341 Replace remaining literal BTQ with `CURRENCY_UNIT` (domob1812)
...
- #16115 On btqd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from btq-cli -getinfo (malevolent)
