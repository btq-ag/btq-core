...
class BTQRegtestMiningTest(BTQTestFramework):
    ...
    def test_regtest_mining(self):
        ...
        assert_equal(blockchain_info['chain'], 'btqregtest')  # Still BTQ
    ...
    def test_generateblock_functionality(self):
        ...
        result = self.generateblock(node, output=address, transactions=[])
        ...
        assert_equal(chain_info['chain'], 'btqregtest')
    ...
    def test_block_generation_with_transactions(self):
        ...
        self.generate(miniwallet, 100)
        
        # Create a BTQ transaction
        address = miniwallet.get_address()
        utxo = miniwallet.get_utxo()
        tx = miniwallet.create_self_transfer(from_node=node, utxo_to_spend=utxo)
        txid = node.sendrawtransaction(tx['hex'])
        
        # Generate BTQ block with this transaction
        result = self.generateblock(node, output=address, transactions=[txid])
