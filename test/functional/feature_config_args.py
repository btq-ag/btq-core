...
class ConfArgsTest(BTQTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in btq.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "btq_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'btq.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "btq_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # Write a btq.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.btqconf.read_text()
        conf_path = default_datadir / "btq.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a btq.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "btq.conf" file which is ignored, because a '
        ...
        # the same btq.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/btq.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when btq.conf in the default data directory sets another datadir '
                      'and it contains a different btq.conf file that would be ignored')
        ...
        # btq.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "btq.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "btq.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/btq.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "btq.conf"
        for chain, chain_name in {("main", ""), ("test", "test"),("signet", "signet")}:
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "btq.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_btq.conf"
