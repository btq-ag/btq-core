...
"""Test objects for interacting with a btqd node over the p2p protocol.

The P2PInterface objects interact with the btqd nodes under test using the
...
MAGIC_BYTES = {
    ...
    "test": b"\x0c\x12\x0a\x08",      # testnet
...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor):
        ...
        logger.debug('Connecting to BTQ Node: %s:%d' % (self.dstaddr, self.dstport))
    ...
    def peer_accept_connection(self, connect_id, connect_cb=lambda: None, *, net, timeout_factor):
        ...
        logger.debug('Listening for BTQ Node with id: {}'.format(connect_id))
...
class P2PInterface(P2PConnection):
    """A high-level P2P interface class for communicating with a BTQ node.
