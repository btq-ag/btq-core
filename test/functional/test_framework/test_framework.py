...
class BTQTestMetaClass(type):
    """Metaclass for BTQTestFramework.

    Ensures that any attempt to register a subclass of `BTQTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BTQTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BTQTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BTQTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BTQTestFramework(metaclass=BTQTestMetaClass):
    """Base class for a btq test script.

    Individual btq test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop btqds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use btq-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "btqd": ("btqd", "BTQD"),
            "btq-cli": ("btqcli", "BTQCLI"),
            "btq-util": ("btqutil", "BTQUTIL"),
            "btq-wallet": ("btqwallet", "BTQWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: btqds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BTQRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a btqd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple btqd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as btqd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in btq.conf
    ...
    def skip_if_no_btqd_tracepoints(self):
        """Skip the running test if btqd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("btqd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_btqd_zmq(self):
        """Skip the running test if btqd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("btqd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if btq-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("btq-wallet has not been compiled")

    def skip_if_no_btq_util(self):
        """Skip the running test if btq-util has not been compiled."""
        if not self.is_btq_util_compiled():
            raise SkipTest("btq-util has not been compiled")
    ...
    def is_cli_compiled(self):
        """Checks whether btq-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether btq-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_btq_util_compiled(self):
        """Checks whether btq-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BTQ_UTIL")
