...
class TestBTQCli(BTQTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `btq-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and btq-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and btq-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BTQ/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BTQ/kvB)']), network_info['relayfee'])
            ...
            # Test btq-cli -generate with -rpcwallet in multiwallet mode.
