...
class FilterTest(BTQTestFramework):
    ...
    def test_msg_mempool(self):
        ...
        txid = self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=filter_peer.watch_script_pubkey, amount=int(0.9 * COIN))["txid"]
    ...
    def test_frelay_false(self, filter_peer):
        ...
        self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=filter_peer.watch_script_pubkey, amount=int(0.9 * COIN))
    ...
    def test_filter(self, filter_peer):
        ...
        filter_peer.tx_received = False
        self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=int(0.7 * COIN))
        filter_peer.sync_with_ping()
        ...
        filter_peer.send_and_ping(msg_filterclear())
        for _ in range(5):
            txid = self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=int(0.7 * COIN))["txid"]
            filter_peer.wait_for_tx(txid)
