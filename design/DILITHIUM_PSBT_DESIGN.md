...
| `0x01` (`SIGHASH_ALL`) | **Accepted** | Default for `.btqms` and v1 tooling |
...
**New helper RPC/CLI:** `btqconvertpsbt <btqms-file> [output.psbt]` (migration).
...
- Sighash golden vectors (§2.5) cross-checked against `btq-multisig` `.btqms` signing
...
6. **Migration:** `.btqms` → PSBT → finalize ≡ legacy finalize hex.
...
| 4 — btq-multisig + migration | 1.5 wk | `--format psbt`; `btqconvertpsbt` |
